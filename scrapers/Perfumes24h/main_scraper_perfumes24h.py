        ...
        for product in products:
            ...
            for variant in variants:
                ...
                if fragrance_name and price and link:
                    fragrances.append({
                        'Brand': aux_functions.standardize_names(brand_name),
...
async def main():
    ...
    async with aiohttp.ClientSession() as session:
        for brand, url in unique_brand_urls:  # Limiting to first 10 for testing
            try:
                response = await aux_functions.fetch(session, url)
                soup = BeautifulSoup(response, 'html.parser')
    ...
    if all_data:
        ...
        aux_functions.scrape_to_db(full_df, 'PT_fragrances', 'Perfumes24h')
